...
# build mesa
meson setup --reconfigure -Dgallium-drivers=etnaviv -Dvulkan-drivers= -Dplatforms= -Degl=enabled -Dglx=disabled -Dglx-direct=false -Ddefault_library=static --cross-file ../src/gk-userland/meson_cross_file.txt -Dbuildtype=release -Db_ndebug=true -Dcmake_prefix_path=$SYSROOT/usr/lib/cmake -Dshader-cache=enabled -Dshader-cache-default=true -Ddri-drivers-path=lib -Dgles-lib-suffix=-base --prefix $SYSROOT/usr build-gkrepo/mesa ../src/mesa
...
cmake $CMAKE_OPTS -DUNIX=ON -DSDL_LIBC=ON -DSDL_PTHREADS=ON -DSDL_THREADS=ON -DSDL_OPENGL=ON -DSDL_OPENGLES=OFF -DSDL_CLOCK_GETTIME=ON -DSDL_LOADSO=ON -S ../src/gk-userland/SDL2-2.28.5/ -B build-gkrepo/sdl2gl

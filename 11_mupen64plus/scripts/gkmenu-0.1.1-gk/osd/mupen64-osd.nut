...
for(local i = 1; i <= 10; i++)
{
    ...
    cbtn.onclick = function(b)
    {
        ...
        [
            { text = "OK", onclick = function(b) { sendkeypress(curkey); sendkeypress(GK_SCANCODE_F5); } },
            { text = "Cancel" }
        ]
        );
    };
...
for(local i = 1; i <= 10; i++)
{
    ...
    cbtn.onclick = function(b)
    {
        ...
        [
            { text = "OK", onclick = function(b) { sendkeypress(curkey); sendkeypress(GK_SCANCODE_F7); } },
            { text = "Cancel" }
        ]
        );
    };
...
b_pause.h = 48;
b_pause.text = "Pause";
b_pause.onclick = function(b) { sendkeypress(GK_SCANCODE_P); };
...
b_quit.h = 48;
b_quit.text = "Quit";
b_quit.onclick = function(b) { sendkeypress(GK_SCANCODE_ESCAPE); delaykill(); };

pause <- function() { sendkeypress(GK_SCANCODE_P); }
